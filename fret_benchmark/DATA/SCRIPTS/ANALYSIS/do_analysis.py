...
for test_dir in test_dirs:
 for line in open(dir+"/"+test_dir+"/best_posterior.dat"):
     ...
     drms_ter=d["DRMS-ter"]
     #sigma0=d["Sigma0"]
     pa=d["Placement_angle"]/math.pi*180.
...
for nunits in [3,4]:
 for sigma in [0.001,0.01]:
  for ndata in [0.5,1.0]:
    id=(nunits,sigma,ndata)
    log.write("%4d %6.4lf %2.1f   " % (id[0],id[1],id[2]))
    ...
    #log.write("%12.6lf  " % (math.sqrt(ave_kda[id]/ntests[id])))
    #log.write("%12.6lf  " % (math.sqrt(ave_Ida[id]/ntests[id])))
    #log.write("%12.6lf  " % (math.sqrt(ave_sigma0[id]/ntests[id])))
    log.write("ntests %4d\n"        % (ntests[id]))
